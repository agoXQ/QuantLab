...
// mirrors notification/infrastructure/usersync verbatim so the cross-
...
const DefaultGroup = "notification-strategy-sync"
...
func dispatch(ctx context.Context, handler domsync.Handler, raw []byte) error {
	...
	case domsync.EventStrategyForked:
		var p domsync.ForkedPayload
		if err := remarshal(env.Payload, &p); err != nil {
			return fmt.Errorf("decode forked payload: %w", err)
		}
		return handler.OnForked(ctx, env, p)
	default:
		return nil
	}
}

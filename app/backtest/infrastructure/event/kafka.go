// Package event provides a Kafka-backed implementation of the Backtest
// domain event publisher. Mirrors the Market Data shape so consumers can
// rely on a single envelope structure.
package event
...
func NewKafkaPublisher(brokers []string) domevent.Publisher {
	return &kafkaPublisher{
		writer: &kafka.Writer{
			...
			Topic:        domevent.TopicBacktestEvents,
			Balancer:     &kafka.Hash{},
...
// Noop is a publisher that drops events on the floor; useful in tests and
